...
class LimitedInteger @kotlin.jvm.JvmOverloads constructor(
	@field:Setter @field:Getter private val limit: Int, @field:Getter private var value: Int = 0
) {
	 ...
	 */
	fun incrementIfLimited(increment: Int): Boolean {
		if (value >= limit) {
			return false
		}
		value += increment
	 ...
	 */
	fun incrementAndCheck(increment: Int): Boolean {
		value += increment
		return value >= limit
	}
	 ...
	 */
	fun incrementIfPossible(increment: Int): Boolean {
	 ...
	 */
	fun isPossible(increment: Int): Boolean {
